...
Processor is the message handler. Here we use the handler of JSON, the default message encoding, and create a Room message.
...
Every message sent from client to server will be flown to `gate` module for routing. Just in brief, `gate` determines which message will be handled by which modules. We want to feed `game` module with `Room` here, so open LeafServer gate/router.go and write below:
...
It is ready to handle `Room` message in `game` module. Open LeafServer game/internal/handler.go and write:
...
func init() {
    // Register the handler of `Room` message to `game` module handleRoom
